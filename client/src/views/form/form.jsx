...
const Form = () => {
    ...
    const submitHandler = (event) => {
        ...
        axios.post("http://localhost:3001/dog", form)
        .then(res => alert(res))
        .catch(err => alert(err))
    }
    ...
    return(
        <div className={style.formContainer} onSubmit={submitHandler} >
            <form className={style.container}  >
                ...
                <div className={style.contentContainer} >
                    <label> Height: </label>
                    <input className={style.input} type='text' value={form.height} onChange={changeHandler} name='height' />
                </div>
                    {errors.height && <span className={style.span} > {errors.height} </span> }
                <div className={style.contentContainer} >
                    <label> Weight: </label>
                    <input className={style.input} type='text' value={form.weight} onChange={changeHandler} name='weight' />
                </div>
                    {errors.weight && <span className={style.span} > {errors.weight} </span> }
                <div className={style.contentContainer} >
                    <label> Life Span: </label>
                    <input className={style.input} type='text' value={form.lifeSpan} onChange={changeHandler} name='lifeSpan' />
                </div>
                    {errors.lifeSpan && <span className={style.span} > {errors.lifeSpan} </span> }
                <div className={style.contentContainer} >
                    <label> Temperaments: </label>
                    <input className={style.input} type='text' value={form.temperaments} onChange={changeHandler} name='temperaments' />
                </div>
                    {errors.temperaments && <span className={style.span} > {errors.temperaments} </span> }
